namespace Cove.GodotFormat
{
    ...
    public class GodotReader(byte[] data, ILogger<GodotReader> logger)
    {
        ...
        public Dictionary<string, object> ReadPacket()
        {
            ...
            {
                logger.LogError(ex, "Error reading packet");
        ...
        private Dictionary<string, object> ReadDictionary()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                {
                    logger.LogError("Error: Dictionary key is not a string at position {Position}", _reader.BaseStream.Position);
                    break;
                }
