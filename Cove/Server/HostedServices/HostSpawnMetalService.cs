...
namespace Cove.Server.HostedServices
{
  ...
  public class HostSpawnMetalService(ILogger<HostSpawnMetalService> logger, CoveServer server) : IHostedService, IDisposable
  {
    private readonly ILogger<HostSpawnMetalService> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
